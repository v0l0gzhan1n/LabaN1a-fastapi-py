...
def test_get_users():
    headers = {"Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjMiLCJleHAiOjE3Mzk2NTU2MjN9.lzobmfk6B7O_QVU6PAKlF9i877zcstjC5H3NXgrVlHA"} 
    response = client.get("/users/", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) > 0
    assert data[0]["username"] == "string7"
...
def test_delete_user():
    headers = {"Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjMiLCJleHAiOjE3Mzk2NTU2MjN9.lzobmfk6B7O_QVU6PAKlF9i877zcstjC5H3NXgrVlHA"}
    response = client.delete("/users/396", headers=headers)
    assert response.status_code == 200
    response = client.delete("/users/396", headers=headers)
